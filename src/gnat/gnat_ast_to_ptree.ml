...
let is_unqualified (node: identifier_id) =
  match List.length (strings_of_name (name_of_identifier node)) with
  | 0 -> failwith "is_unqualified: empty"
  | 1 -> true
  | _ -> false
...
let mk_theory_declaration (node : theory_declaration_id) =
  let Theory_declaration r = node.desc in
...
let mlw_file nodes =
  Modules (List.map mk_theory_declaration nodes)
...
let read_channel env path filename c =
  ...
  let gnat_ast =
    try Gnat_ast.From_json.theory_declaration_opaque_olist_from_json json
