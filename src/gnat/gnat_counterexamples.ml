...
let clean = object (self)
  ...
  method! element me =
    ...
    | Some me ->
        let men_name =
          Ident.get_model_trace_string ~name:me.me_name.Model_parser.men_name
            ~attrs:me.me_name.Model_parser.men_attrs in
        let men_name = List.hd (Strings.bounded_split '@' men_name 2) in
        let me = {me with me_name= clean_name {me.me_name with men_name}} in
