...
let add_opt_trans x =
  let name_args =
    ...
    | [] -> eprintf "Empty transformation for --apply-transformation"; exit 1
    | [name] -> name, None
    | name :: args -> name, Some args in
  opt_trans := name_args :: !opt_trans
...
let do_tasks env drv fname tname th task =
  ...
  let rec apply tasks = function
    ...
    | (name, args) :: trans ->
        let apply_trans =
          match args with
          | None -> Trans.apply_transform name env
          | Some args ->
